...
def aes_ctr_decrypt(k, ct, nonce):
    """
    Encrypts a message using AES in CTR mode. The nonce is 8-byte and
    little-ending. The counter is similarly 8-byte and little-endian. The input
    into the AES block cipher primitive is AES(key, nonce||ctr). For example,
    with an original nonce of:
    ...
    blocksize = 16
    if len(ct) > 256 ** 8:
        raise Exception('CT size is too long.')
    if len(k) != blocksize:
...
if __name__=='__main__':
    ...
    s = ('L77na/nrFsKvynd6HzOoG7GHTLXsTVu9qvY/2syLXzhPweyyMTJULu/6/kXX0KSvoOLS'
         'FQ==')
    print aes_ctr_decrypt(k, b642hex(s).decode('hex'), '\x00'*(len(k)/2))
